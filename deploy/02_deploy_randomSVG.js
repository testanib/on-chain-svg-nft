...
module.exports = async ({
    ...
    log("----------------------------------------------------")
    const RandomSVG = await deploy('RandomOnChainSVG', {
        from: deployer,
        args: args,
        log: true
    })
    ...
    const RandomSVGContract = await ethers.getContractFactory("RandomOnChainSVG")
    ...
    tx = await randomSVG.create({ gasLimit: 400000, value: '10000000000000000' })
    ...
    if (chainId != 31337) {
        ...
        tx = await randomSVG.finishMint(tokenId, { gasLimit: 3000000, gasPrice: 3000000017 })
        await tx.wait(1)
        ...
        log(`Now let's finsih the mint...`)
        tx = await randomSVG.finishMint(tokenId, { gasLimit: 4000000, gasPrice: 3000000017 })
        await tx.wait(1)
        log(`You can view the tokenURI here ${await randomSVG.tokenURI(0)}`)
    } */
}
